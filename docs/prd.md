...
The three models already work in isolation. The value of putting them behind one agent comes from the fact that churn, fraud and service quality are owned by different teams inside an operator, funded from different budgets, and rarely reconciled against each other. What the agent adds over the three models running separately: root cause across domains in a single query. Ranking by revenue at risk, and fraud triage that weighs customer value, are operator-facing story only in v1 — not a shipped ranking feature.
...
5. The agent presents insight: where the problem is concentrated, whether QoE or fraud is a factor, how many customers are at risk, and a suggested next action (for example investigate coverage or target retention offers). The agent suggests; it does not execute retention offers or account blocks.
...
- "Are fraud-flagged customers also churning?" — overlap of fraud flags and churn risk; review before blocking accounts.
- "Which region should we worry about most?" — QoE, churn, and fraud together to name a priority region.
...
- **customers** — churn profile from the churn dataset (existing `CustomerID` is the join key), plus location (`lat`/`lon`), region, and `season` / `weather` for QoE. Plan type and similar metadata as needed to slice the agent queries.
...
- Draw **original** Starlink feature tuples (`latitude`, `longitude`, `season`, `weather`) and assign them to customers, grouped into regions so nearby locations form an area and QoE can vary by region. Do not invent coordinates outside that empirical distribution.
...
- **Churn scoring (v1):** a `CustomerID` has several billing-cycle rows. Dashboard scoring and `predict_churn` use the **latest `BillingCycleStart` per customer**. All cycle rows stay in the store. The agent does **not** pick a month in v1. Future: `predict_churn` may take a month so the agent can score a specific cycle.
...
**Agent tools hide serving.** The agent does not call a model HTTP API and does not know Flask vs KServe, path names, or wire formats. `predict_churn`, `predict_fraud`, and `predict_qoe` own that connection: they take domain inputs (customer / transaction / location fields) and return domain results (e.g. Will Churn / Will Not Churn, Fraud / Non-Fraud, QoE numbers). If a model is later served with KServe V2, the tool — not the agent — maps JSON fields into the V2 tensor payload and maps numeric outputs (0 / 1) back to labels. Swapping a backend must not change the agent or the tool schemas.
...
- LLM: specific model left to architecture. Default is on-cluster if the cluster can host it (source note sized around a single L4/A10). If the cluster cannot support that, use a remote endpoint. GPU is optional, not a hard requirement.
...
- Add a live weather/season tool (season and weather come from assigned Starlink tuples / generated customer fields)
...
- Impose extra compliance, PII, data-residency, or agent-audit requirements
- Ship the existing Starlink interactive map UI as the product dashboard (v1 dashboard is summary metrics + chatbot)
- Ship ranking by revenue at risk or customer-value fraud triage as a distinct feature (operator story only)
- Claim CFCA / TM Forum / carrier churn dollar figures as quickstart outcomes
...
v1 WILL document the planted QoE–churn regional coupling as intentional and tunable, and will not hard-code that the outlier region is West.
